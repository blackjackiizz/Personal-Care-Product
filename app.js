...
async function loadRecords() {
  try {
    const res = await fetch(API, { method: "POST", body: JSON.stringify({ action: "getAllRecords", mode }) });
...
document.getElementById("loadData").onclick = async () => {
  ...
  const res = await fetch(API, { method: "POST", body: JSON.stringify({ action: "getPriceHistory", mode, item }) });
